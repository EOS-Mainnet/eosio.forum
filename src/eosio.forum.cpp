...
[[eosio::action]]
void forum::propose(const name proposer,const name proposal_name,const string& title,const string& proposal_json,const time_point_sec& expires_at) {
...
[[eosio::action]]
void forum::vote(const name voter,const name proposal_name,uint8_t vote,const string& vote_json) {
    require_auth(voter);
...
[[eosio::action]]
void forum::post(const name poster,const string& post_uuid,const string& content,const name reply_to_poster,const string& reply_to_post_uuid,const bool certify,const string& json_metadata) {
    require_auth(poster);
...
void forum::update_status(statuses& status_table,const name account,const function<void(status_row&)> updater) {
...
void forum::update_vote(votes& vote_table,const name proposal_name,const name voter,const function<void(vote_row&)> updater) {
    auto index = vote_table.template get_index<"byproposal"_n>();
    auto vote_key = compute_by_proposal_key(proposal_name, voter);
...
void forum::validate_json(const string& payload,size_t max_size,const char* not_object_message,const char* over_size_message) {
